...
export function Layout({ children, currentView, setCurrentView, emoteStats }: LayoutProps) {
    const { update, downloading, downloadProgress, installUpdate } = useUpdate();
    const [dismissedVersion, setDismissedVersion] = useState<string | null>(
        () => localStorage.getItem('dismissed-update')
    );

    const showBanner = !!update && update.version !== dismissedVersion;
                ...
                {showBanner && (
                    <div className="flex items-center gap-3 px-4 py-2.5 bg-yellow-400/10 border-b border-yellow-400/20 text-sm shrink-0">
                        ...
                        <span className="text-yellow-300 font-medium flex-1">
                            VOX_TERMINAL v{update!.version} is available
                            {update!.body ? ` — ${update!.body}` : ''}
                        </span>
                        <button
                            onClick={() => void installUpdate()}
                            disabled={downloading}
                            className="px-2.5 py-1 bg-yellow-400 text-black text-xs font-bold rounded-md hover:bg-yellow-300 transition-colors shrink-0 disabled:opacity-60"
                        >
